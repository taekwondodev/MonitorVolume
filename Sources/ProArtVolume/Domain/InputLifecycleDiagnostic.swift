...
struct InputLifecycleTraceAssessment: Equatable {
    let hasGapsOrInvalidPairs: Bool
    let budgetExhausted: Bool
    let ended: Bool
    let unfinishedOperations: Set<UInt64>

    var isComplete: Bool {
        !hasGapsOrInvalidPairs && !budgetExhausted && ended && unfinishedOperations.isEmpty
    }

    static func assess(_ records: [InputLifecycleRecord]) -> Self {
        var expected: UInt64 = 1
        var previousTime: UInt64 = 0
        var invalid = records.first?.event != .sessionStarted
        var exhausted = false
        var ended = false
        var pending: [UInt64: InputLifecycleEvent] = [:]
        for record in records {
            if record.sequence != expected || record.uptimeNanoseconds < previousTime || ended || exhausted {
                invalid = true
            }
            expected = record.sequence &+ 1
            previousTime = record.uptimeNanoseconds
            switch record.event {
            case let .operationBegan(token, _):
                if token.sequence != record.sequence || pending.updateValue(record.event, forKey: token.sequence) != nil {
                    invalid = true
                }
            case let .queryBegan(token):
                if token.sequence != record.sequence || pending.updateValue(record.event, forKey: token.sequence) != nil {
            ...
            case let .operationEnded(token, _):
                if case let .operationBegan(start, _) = pending.removeValue(forKey: token.sequence) {
                    if start != token { invalid = true }
                } else { invalid = true }
            case let .queryReturned(token, _):
                if case let .queryBegan(start) = pending.removeValue(forKey: token.sequence) {
                    if start != token { invalid = true }
                } else { invalid = true }
            case .budgetExhausted: exhausted = true
            case .sessionEnded: ended = true
            case .sessionStarted:
                if record.sequence != 1 { invalid = true }
            default: break
            }
        }
        return Self(hasGapsOrInvalidPairs: invalid, budgetExhausted: exhausted,
                    ended: ended, unfinishedOperations: Set(pending.keys))
    }
}
